...
{
	return m_pTouchControls->m_EditingActive || std::all_of(m_vVisibilities.begin(), m_vVisibilities.end(), [&](CButtonVisibility Visibility) {
...
void CTouchControls::UpdateButtons(const std::vector<IInput::CTouchFingerState> &vTouchFingerStates)
{
	...
	// Activate visible, inactive buttons with hovered finger. Deactivate previous button
	// being activated by the same finger.
	for(CTouchButton &TouchButton : m_vTouchButtons)
	{
		...
		const auto FingerInsideButton = std::find_if(vRemainingTouchFingerStates.begin(), vRemainingTouchFingerStates.end(), [&](const IInput::CTouchFingerState &TouchFingerState) {
			return TouchButton.IsInside(TouchFingerState.m_Position * ScreenSize);
		});
